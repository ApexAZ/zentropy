...
(function (): void {
	...
	function renderTeams(): void {
		...
		<div class="team-card" data-team-id="${team.id}">
			...
			<div class="team-footer">
				...
				<button class="btn btn-outline btn-small" data-action="view-team-details" data-team-id="${team.id}">
					View Details
				</button>
			</div>
		</div>
	...
	function editTeam(teamId: string): void {
		try {
			...
			(document.getElementById("velocity-baseline") as HTMLInputElement).value =
				team.velocity_baseline.toString();
			(document.getElementById("sprint-length") as HTMLSelectElement).value = team.sprint_length_days.toString();
			(document.getElementById("working-days") as HTMLSelectElement).value =
				team.working_days_per_week.toString();
		...
		try {
			...
			if (!response.ok) {
				const errorData = (await response
					.json()
					.catch((): ValidationError => ({ message: "Unknown error occurred" }))) as ValidationError;
			...
			renderTeams();
			closeTeamModal();
			showToast(wasEditing ? "Team updated successfully!" : "Team created successfully!", "success");
		} catch (error) {
			// eslint-disable-next-line no-console
	...
	document.addEventListener("click", (event: Event) => {
		...
		switch (action) {
			case "show-create-team-modal":
				showCreateTeamModal();
				break;
			case "edit-team":
				editTeam(target.dataset.teamId ?? "");
				break;
			case "confirm-delete-team":
				confirmDeleteTeam(target.dataset.teamId ?? "");
				break;
			case "view-team-details":
				viewTeamDetails(target.dataset.teamId ?? "");
				break;
