...
function renderCalendar(): void {
	...
	for (let week = 0; week < 6; week++) {
		for (let day = 0; day < 7; day++) {
			...
			if (dayEntries.length > 0) {dayClass += ' has-entries';}
...
function renderEntriesList(): void {
	...
	entriesList.innerHTML = monthEntries.map((entry: CalendarEntry) => {
		const user = users.find(u => u.id === entry.user_id);
			...
			<div class="entry-item" data-entry-id="${entry.id}">
				<div class="entry-header">
					...
					<div class="entry-actions">
						<button class="btn-icon" onclick="editEntry('${entry.id}')" title="Edit Entry">
							<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
								<path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/>
							</svg>
						</button>
						<button class="btn-icon btn-danger" onclick="confirmDeleteEntry('${entry.id}')" title="Delete Entry">
							<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
...
function calculateCapacityImpact(): void {
	...
	document.getElementById('impact-days')!.textContent = workingDays.toString();
	document.getElementById('impact-percentage')!.textContent = `${percentage}%`;
...
(window as any).confirmDeleteEntry = confirmDeleteEntry;
