...
describe("Semantic Color Validation", () => {
	...
	const getComponentFiles = (): string[] => {
		const componentDirs = ["src/client/components", "src/client/pages"];
	...
	it("should use semantic color variables in all components", () => {
		...
		if (violations.length > 0) {
			const errorMessage = violations.map(v => `${v.file}: ${v.issues.join(", ")}`).join("\n");

			expect.fail(
				`Found hardcoded colors in components:\n${errorMessage}\n\nPlease use semantic color classes: ${semanticClasses.join(", ")}`
			);
		}
	...
	it("should find semantic color usage in components", () => {
		...
		componentFiles.forEach(filePath => {
			...
			semanticClasses.forEach(semanticClass => {
				const usageCount = classes.filter(
					cls =>
		...
		componentFiles.forEach(filePath => {
			...
			classes.forEach(className => {
				...
				if (className.includes("layout-bg") || className.includes("content-bg")) {
					invalidSemanticUsage.push(
						`${path.basename(filePath)}: ${className} (should use bg-layout-background or bg-content-background)`
					);
				}

				if (className.includes("primary-text") || className.includes("contrast-text")) {
					invalidSemanticUsage.push(
						`${path.basename(filePath)}: ${className} (should use text-primary or text-contrast)`
					);
				}
			});
		});
