    ...
    return (
      <main className="w-full py-8">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h2 className="text-3xl font-semibold text-gray-800 m-0">Team Calendar</h2>
            <p className="text-gray-600 mt-2">Manage team calendar entries and PTO for accurate sprint capacity planning</p>
          </div>
        </div>
        <div className="flex flex-col items-center justify-center min-h-[300px] text-center">
          <div className="w-10 h-10 border-4 border-gray-200 border-t-blue-500 rounded-full animate-spin mb-4"></div>
          <p className="text-gray-600 mb-4">Loading calendar entries...</p>
        </div>
      </main>
    )
  }
  ...
  if (error) {
    return (
      <main className="w-full py-8">
        ...
        <div className="flex flex-col items-center justify-center min-h-[300px] text-center">
          <div>
            <svg className="w-12 h-12 text-red-600 mb-4 mx-auto" viewBox="0 0 24 24" fill="currentColor">
  ...
  return (
    <main className="w-full py-8">
      ...
      <div className="flex justify-between items-center mb-6 p-4 bg-white rounded-lg shadow-sm border border-gray-200">
        <div className="flex gap-4">
          <select
            ...
            className="p-2 border border-gray-300 rounded-md text-base transition-all duration-200 focus:outline-none focus:border-blue-500 focus:shadow-[0_0_0_3px_rgba(59,130,246,0.1)]"
          >
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
        <div className="p-4">
          <p className="text-gray-500 text-center py-8">Calendar grid view coming soon...</p>
        </div>
      </div>

      {/* Entries List */}
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h3 className="text-xl font-semibold text-gray-800 mb-4">Upcoming Entries</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6">
              <div className="space-y-6">
                <div className="flex flex-col gap-2">
                  ...
                  {formErrors.team_id && (
                    <span className="text-sm text-red-600 mt-1">{formErrors.team_id}</span>
                  )}
                </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <select
                    ...
                    className="w-full p-3 border border-gray-300 rounded-md text-base leading-6 transition-all duration-200 focus:outline-none focus:border-blue-500 focus:shadow-[0_0_0_3px_rgba(59,130,246,0.1)] disabled:bg-gray-100 disabled:cursor-not-allowed"
                    required
                  >
                    <option value="">{formData.team_id ? 'Select a team member' : 'Select team first'}</option>
                    {teamUsers.map(user => (
                      <option key={user.id} value={user.id}>{user.first_name} {user.last_name}</option>
                    ))}
                  </select>
                  {formErrors.user_id && (
                    <span className="text-sm text-red-600 mt-1">{formErrors.user_id}</span>
                  )}
                </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  />
                  {formErrors.title && (
                    <span className="text-sm text-red-600 mt-1">{formErrors.title}</span>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="flex flex-col gap-2">
                    ...
                    {formErrors.start_date && (
                      <span className="text-sm text-red-600 mt-1">{formErrors.start_date}</span>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    />
                    {formErrors.end_date && (
                      <span className="text-sm text-red-600 mt-1">{formErrors.end_date}</span>
                    )}
                  </div>
                </div>
              ...
              <div className="flex justify-end gap-4 p-6 border-t border-gray-200 mt-6">
                <button
                  type="button"
                  onClick={closeModals}
                  className="inline-flex items-center gap-2 py-2 px-4 bg-white text-gray-700 border border-gray-300 rounded-md text-base font-medium text-center no-underline cursor-pointer transition-all duration-200 hover:bg-gray-50 hover:border-gray-400"
      ...
      {showDeleteModal && currentEntry && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-6">
          <div className="bg-white rounded-lg shadow-lg max-w-md w-full max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center p-6 border-b border-gray-200">
              <h3 className="m-0 text-xl font-semibold text-gray-900">Delete Calendar Entry</h3>
              <button
                onClick={closeModals}
                className="bg-none border-none text-2xl text-gray-400 cursor-pointer p-2 rounded-md transition-colors duration-200 hover:text-gray-600"
              >
            ...
            <div className="p-6">
              ...
              <p className="text-gray-700 font-medium">
                {currentEntry.title} ({formatDate(currentEntry.start_date)} - {formatDate(currentEntry.end_date)})
              </p>
            </div>
