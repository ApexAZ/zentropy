"""
Add microsoft_oauth to RegistrationType enum

This migration adds the 'microsoft_oauth' value to the registrationtype enum
to support Microsoft OAuth user registration tracking.
...
def upgrade():
    """Add microsoft_oauth value to registrationtype enum"""
    ...
    op.execute("ALTER TYPE registrationtype ADD VALUE 'microsoft_oauth'")

    print("✅ Added 'microsoft_oauth' to registrationtype enum")
    print("🔐 Microsoft OAuth users can now be properly tracked")


def downgrade():
    """Remove microsoft_oauth value from registrationtype enum"""
    ...
    op.execute(
        "CREATE TYPE registrationtype_new AS ENUM ('email', 'google_oauth')"
    )

    # 2. Update all columns to use new enum
    # (this will fail if microsoft_oauth values exist)
    op.execute(
