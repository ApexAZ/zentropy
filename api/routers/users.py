...
from ..auth import get_current_active_user, get_password_hash, verify_password, validate_password_strength
...
def get_current_user_profile(current_user: database.User = Depends(get_current_active_user)):
...
def change_password(
    ...
    password_history = db.query(database.PasswordHistory).filter(
        database.PasswordHistory.user_id == current_user.id
    ).order_by(database.PasswordHistory.created_at.desc()).limit(5).all()
    
    for history_entry in password_history:
        if verify_password(password_update.new_password, history_entry.password_hash):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    old_entries = db.query(database.PasswordHistory).filter(
        database.PasswordHistory.user_id == current_user.id
    ).order_by(database.PasswordHistory.created_at.desc()).offset(5).all()
